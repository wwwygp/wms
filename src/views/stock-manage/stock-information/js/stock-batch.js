...
import {
  queryStockInformation,stockExportToExcelByBatch,getCommodityInfoPageBySpace,getCommodityInfoPageByBatch,getTestReportUrl
...
export default {
  ...
  data() {
    return {
      ...
      tableName: {
        tableCode: 'WMS_STOCK_INFORMATION_BY_BATCH'
      },
  ...
  methods: {
    ...
    initStock(current) {
      ...
      getCommodityInfoPageByBatch(this.filteParams(params)).then(res => {
        let re = res.data;
        if (re.status == 10001) {
    ...
    exportStockByBatch(){
      ...
      window.location.href = this.setQueryConfig("/apiz/rest/wms/stock/v1/export-stock-by-batch?", params)
    },
