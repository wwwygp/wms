...
import {printerGroupAdd,printerGroupUpdate,printerGroupDel} from '@/api/basic-data/printer-group/index.js';
...
export default {
  ...
  data() {
    return{
      ...
      rulesForm: {
        printerGroupCode: [{required: true,message: '编码不能为空',trigger: 'blur'}],
        printerGroupName: [{required: true, message: '名称不能为空',trigger: 'blur'}],
      },
      printerStatus: [],
      printerType: [],
      printerIds: '',
  ...
  methods: {
    ...
    initBtn () {
      let menusKey = 'WMS_PRINTER_GROUP';
    ...
    initPrinter (current) {
      ...
      printerGroup(this.filteParams(params)).then(res => {
        // this.dataTable.loading = false;
        let re = res.data;
        console.log(re,'123.123')
        this.printerNames = [];
    ...
    searchList () {
      this.searchParam = {
        printerGroupName: this.formInline.printerGroupName,
        printerGroupCode: this.formInline.printerGroupCode,
      }
      this.initPrinter(1);
    },
    resetList () {
      this.formInline.printerGroupName = '';
      this.formInline.printerGroupCode = '';
    ...
    updateForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let axiosApi;
          this.printerTitle == '修改' ? axiosApi = printerGroupUpdate(this.form): axiosApi = printerGroupAdd(this.form);
          axiosApi.then(res => {
    ...
    delList () {
      this.getSelectedRow.length != 0 ? this.delDialog.modalShow = true: this.$message.warning('请勾选要删除的数据');
    },
    removeRow () {
      let printerGroupIdArr = [];
      this.getSelectedRow.forEach((item,index) => {
        printerGroupIdArr.push(item.printerGroupId);
      });
      this.printerGroupIds = printerGroupIdArr.join(',');
      printerGroupDel(this.printerGroupIds).then(res => {
        if (res.data.message) {
