...
import { getProcessPlanDetail } from '@/api/processing-management/inbound-process/inbound-process-plan-detail'
...
export default {
  ...
  created () {
    ...
    let processPlanDetail =  JSON.parse(window.localStorage.getItem("processPlanDetail"))
    if(processPlanDetail && processPlanDetail.processPlanCode){
      this.formInline = processPlanDetail
    }
  },
  watch: {
    'dataTableDialog.data':{
      handler(newValue, oldValue) {
        newValue.forEach((item, index) => {
          let reg = /^(-|\+)?\d+$/
          if(item.smallPackageUnitId != 10 && !(reg.test(item.stayCol1))) {
  ...
  methods: {
    ...
    movePlanDtlSaveCommodity(){
      ...
      for (var i = 0; i < this.getSelectedRowDialog.length; i++) {
        ...
        //   return flag1 = false
        // }
        if(this.getSelectedRowDialog[i].smallPackageUnitId != 10 && !(reg.test(this.getSelectedRowDialog[i].stayCol1))) {
            this.$message.warning('请输入整数')
            return flag1 = false
          }
    ...
    handleBack(item){
      ...
      //   this.backMovePlan()
      // }
      this.handleBackbackProcessPlan()
    },
    handleBackbackProcessPlan(){
      if(top && top.createTab) {
          this.routeERP("WMS_INBOUND_PROCESS_PLAN", '入库加工计划页', '/iwms/#/inbound_process_plan')
        }else {
          this.$router.push({
