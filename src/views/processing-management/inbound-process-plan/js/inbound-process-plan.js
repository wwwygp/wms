...
import { getProcessPlanList } from '@/api/processing-management/inbound-process/inbound-process-plan'
...
export default {
  ...
  methods: {
    ...
    //表格高度
    tableHeight() {
      let formHeight = $("#process-plan-form").height();
    ...
    handleRoute(item){
      window.localStorage.setItem("processPlanDetail", JSON.stringify(item))
      if(top && top.createTab){
        this.routeERP("WMS_INBOUND_PROCESS_PLAN_DTL", '入库加工计划明细单', '/iwms/#/inbound_process_plan_detail')
      }else{
        this.$router.push({
