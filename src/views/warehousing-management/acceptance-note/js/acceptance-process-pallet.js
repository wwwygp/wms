...
import { getAcceptancePalletList, getAcceptanceNoteDetailList, addPackingSize} from '@/api/warehousing-management/accepttance-note/accepttance-process-pallet';
import { getContainerList, delAcceptancePallet, checkLabelNumber, getSpaceList } from '@/api/warehousing-management/accepttance-note/accepttance-pallet';
import { dictionarysTypeNew, standardDicPage } from '@/api/common/type.js';//字典返回值，编码规则
...
export default {
  name: 'acceptance-process-pallet',
  components: {
    DatePick,
    tableConfigure,
    selfDialog,
  ...
  data () {
    return{
      ...
      tableName: {
        tableCode: "WMS_ACCEPTANCE_PROCESS_PALLET_DETAIL_TABLE"
      }, // 表格ID ，就是表格自定义名称
      ...
      oldDialogDataTable: [],
      dialogTableName: {
        tableCode: "WMS_WORKABLE_NOTE_DTL"
      },
      title: '',
      form: {
        ownerId: '',
      ...
      packingSizeTitle: '',
      isPackingSizeSave: false,//若有数据新增，未点击保存
      isSavePackingSizeDialog: {//确认是否保存
        modalClickShow: false,
        title: '确认',
        modalShow: false,
        text: '该数据未保存，确定放弃编辑?'
...
　　　deep: true
  ...
  methods: {
    initBtn () {
      let menusKey = 'WMS_ACCEPTANCE_PROCESS_PALLET';
    ...
    searchList(){//查询
      if(this.acceptanceNoteId){
        this.searchParam ={
          ...
          commodityBarcode: this.formInline.commodityBarcode,
          caseCode: this.formInline.caseCode,
          palletCode: this.formInline.palletCode
        }
    ...
    acceptancePallet(){
      if(this.acceptanceNoteId){
        ...
        this.isSave = false
        dictionarysTypeNew('WMS_COMMODITY_TYPE_ID_COMTYPE', this, 'commodityType')
        this.initBrandsList()
        this.initOwnersList()
    ...
    delAcceptancePallet(){//删除
      if(this.getSelectedRow.length == 0){
        this.$message.warning('请选择需要删除的验收加工板明细信息');
        return false
      }else{
    ...
    //表格高度
    tableHeight() {
      let formHeight = $("#process-process-pallet-form").height();
    ...
    initDialogTable (current) { // 初始化表格
      ...
      //合并缓存查询条件
      this.extendJson(params, this.searchDialogParam)
    ...
    dialogTableDataHandle(data) { // table数据处理函数
      ...
      handleTableData.forEach((item, index) => { // 写表格api的时候建议加个字段。字段内容是每个表头对应的key,这样prop就可以用这个字段
        ...
        item.sortable = false; // 是否要排序
        item.prop = item.propName; // 这个是相应的显示字段
        if (item.prop == 'largePackageNumber' || item.prop == 'mediumPackageNumber'
          || item.prop == 'smallPackageNumber' || item.prop == 'processPackageSpec') {
    ...
    resetDialogForm(){
      ...
      for (let item in this.searchDialogParam) {
        this.searchDialogParam[item] = ''
      }
    ...
    cancelPackingSize(){
      //所有数据重置
      this.showPackingSizeDialog = false
      this.isSavePackingSizeDialog.modalShow = false
      this.isSavePackingSize = false
    },
    comfirmPackingSize(){
      //addPackingSize
      let params = {
        tenantId: this.packingForm.tenantId,
        commodityId: this.packingForm.commodityId,
        largePackageCount: this.packingForm.largePackageCount,
        largePackageUnitId: this.packingForm.largePackageUnitId,
        smallPackageCount: this.packingForm.smallPackageCount,
        smallPackageUnitId: this.packingForm.smallPackageUnitId,
      }
      addPackingSize(params).then(res => {
        let data = res.data
        if(data.status == 10001){
          this.$message.warning(data.message)
        }else{
